  ...
  return (
    <style>{`
      ...
      *{box-sizing:border-box}
      ...
      .ag-btn{border:none;cursor:pointer;font-family:${FF};font-weight:500;font-size:12.5px;border-radius:9px;padding:8px 16px;transition:filter .15s,opacity .15s;display:inline-flex;align-items:center;gap:8px}
      .ag-btn:hover:not(:disabled){filter:brightness(1.12)}
      .ag-btn:disabled{opacity:.45;cursor:not-allowed}
      .ag-nav-row{display:flex;align-items:center;gap:10px;width:calc(100% - 12px);margin:1px 6px;text-align:left;background:none;border:none;border-radius:9px;cursor:pointer;font-family:${FF};font-size:13px;font-weight:400;padding:9px 12px;color:${C.muted};transition:color .12s,background .12s}
      .ag-nav-row:hover{color:${C.ink};background:${C.navyLight}}
      .ag-nav-row.active{color:${C.white};background:${C.accentBg};font-weight:500}
      .ag-link{color:${C.accent};text-decoration:none}
      .ag-link:hover{text-decoration:underline}
      textarea,input,select{font-family:${FF};font-weight:400;color:${C.ink};background:${C.navy};border:1px solid ${C.border};border-radius:10px;padding:10px 12px;outline:none;transition:border-color .15s}
      textarea:focus,input:focus,select:focus{border-color:${C.accent}}
